#!/usr/bin/env python3
"""
Push June edition to ActiveCampaign:
  1. POST /api/3/messages      — create the June message (HTML body)
  2. (v1) campaign_create      — create draft campaign on list 2 (Active Family Office Members)
  3. (v1) campaign_send        — send a TEST send to the confirmed recipients

Leaves the campaign in status=0 (draft). Does NOT send to the list.
Reads AC_API_URL + AC_API_KEY from .env. Writes campaign + message IDs to stdout.
"""
...
HTML_PATH = REPO / "exports/trends-and-topics-june/trends-and-topics-june-v2.html"
ENV_PATH = REPO / ".env"
LIST_ID = 2  # "Active Family Office Members" — verified from campaign 35
SUBJECT = "2^n Family Office Brief: June 2026"
FROM_NAME = "2^n: trends&topics"
FROM_EMAIL = "mb@2pwrn.com"
REPLY_TO = "mb@2pwrn.com"
CAMPAIGN_NAME = "trends&topics (June 2026)"
TEST_RECIPIENTS = ["cj@2pwrn.com", "mb@2pwrn.com", "sydneyb@2pwrn.com"]
...
def main():
    ...
    # ---- 2. Create campaign (draft) — legacy v1 (v3 returns 405 for campaign_create) ----
    ...
    print(f"  → response: {json.dumps(parsed, indent=2)[:800]}")
    if not cmp_id:
    ...
    print("\nDone.")
    print(f"  Campaign #{cmp_id} (DRAFT, status=0) — review: https://2pwrn.activehosted.com/campaign/{cmp_id}/designer")
